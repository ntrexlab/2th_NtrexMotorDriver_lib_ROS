...
void stellaN1_node::command_velocity_callback(const geometry_msgs::msg::Twist::SharedPtr cmd_vel_msg)
{
  if(RUN)
  {
    ...
    float L = -((goal_linear_velocity_ + wheel_to_wheel_d / 2.0 * goal_angular_velocity_) * motor_scale_const);
    float R = -((goal_linear_velocity_ - wheel_to_wheel_d / 2.0 * goal_angular_velocity_) * motor_scale_const);

    dual_m_command(dual_m_command_select::velocity, L, R);
  }
}
...
{
  delta_left = Limit_i((MyMotorCommandReadValue.position[channel_1] - left_encoder_prev) * -1, 70, 100000);
  delta_right = Limit_i((MyMotorCommandReadValue.position[channel_2] - right_encoder_prev) * -1, 70, 100000);
...
int main(int argc, char *argv[])
{
  ...
  MW_Serial_Connect("/dev/ttyUSB0", 115200);
